...
// Time returns the event timestamp in local time.
func (e Event) Time() time.Time {
	return time.UnixMilli(e.UTC*1000 + e.UTCMS).Local()
}
...
type CameraStream struct {
	...
	client    *http.Client
	raw       io.Writer
	onEvent   func(camera string)
	onConnect func()
}

// NewCameraStream validates the config and creates a reusable HTTP client.
func NewCameraStream(cam CameraConfig) (*CameraStream, error) {
	if cam.Name == "" {
	...
	if cam.User == "" {
		return nil, fmt.Errorf("camera %q: user is required", cam.Name)
	}
	base := &http.Transport{
...
func (cs *CameraStream) Name() string         { return cs.cam.Name }
func (cs *CameraStream) Config() CameraConfig { return cs.cam }
...
func (cs *CameraStream) SetOnEvent(fn func(string)) { cs.onEvent = fn }
...
func (cs *CameraStream) Connect(ctx context.Context, store *Store) error {
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, cs.cam.URL, nil)
	if err != nil {
		return err
	}

	slog.Info("connecting", "camera", cs.cam.Name, "url", cs.cam.URL)
	...
	slog.Info("connected", "camera", cs.cam.Name, "status", resp.StatusCode)
	if cs.onConnect != nil {
		cs.onConnect()
	}
	...
	return iterLines(body, cs.cam.Name, func(e Event) {
		if err := store.Insert(e); err != nil {
			slog.Error("insert failed", "camera", cs.cam.Name, "plate", e.Plate, "err", err)
			return
		}
		if cs.onEvent != nil {
			cs.onEvent(e.Camera)
