...
async function refresh() {
  try {
    ...
    for (const cam of d.cameras) {
      ...
      row.querySelector('.plate').textContent = cam.last_plate || '-';
    }
...
func (s *cameraStatus) touchPlate(plate string) {
	s.mu.Lock()
	s.lastPlate = plate
	s.mu.Unlock()
}
...
func NewStatusServer(streams []*CameraStream, store *Store) *StatusServer {
	...
	if latest, err := store.QueryLatestPerCamera(hs.names); err == nil {
		for name, plate := range latest {
			if s, ok := hs.statuses[name]; ok {
				s.lastPlate = plate
			}
		}
	}
...
func (hs *StatusServer) Touch(camera, plate string) {
